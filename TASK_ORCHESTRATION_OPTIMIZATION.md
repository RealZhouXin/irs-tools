...
### 优化方案
引入标签系统（Tagging）控制执行范围，引入树形层级结构（Hierarchical）优化 UI 展示。

### 实施步骤
#### 1. 引入标签系统 (Tagging)
标签系统是解决“多场景复用同一份配置”的最佳方案。

* **修改配置文件 (`tests.toml`)**：为每个测试项增加一个 `tags` 数组字段。
  ```toml
  [[tests]]
  name = "068 主板应用软件"
  command = "param_id068"
  tags = ["quick_check", "software", "production"] # 打上标签
  # ... checks ...

  [[tests]]
  name = "272 电池信息"
  command = "param_id272"
  tags = ["battery", "production"] # 电池模块、生产线必测
  # ... checks ...

  [[tests]]
  name = "588 电机高负荷测试"
  command = "param_id588"
  tags = ["motor", "stress_test"] # 电机模块、压力测试（耗时较长）
  # ... checks ...
  ```
* **修改 Rust 数据模型 (`models.rs`)**：在 `TestGroup` 结构体中增加 `tags` 字段。
  ```rust
  #[derive(Debug, Deserialize, Serialize, Clone)]
  pub struct TestGroup {
      pub name: String,
      #[serde(default)] // 如果 toml 里没写 tags，默认为空数组
      pub tags: Vec<String>, 
      #[serde(flatten)]
      pub command: CommandGroupSpec,
  }
  ```
* **修改后端执行逻辑 (`test_service.rs` & `commands.rs`)**：修改 `start_test` 命令，允许前端传入一个可选的标签列表。后端在执行前，先对 `tests` 数组进行过滤。
  ```rust
  // commands.rs
  #[tauri::command]
  pub async fn start_test(
      app: tauri::AppHandle, 
      run_tags: Option<Vec<String>> // 前端传来的标签过滤条件
  ) -> CommandResult<TestSummary> {
      // ...
      TestService::new(app_handle).start_test(run_tags)
  }

  // test_service.rs
  pub fn start_test(&self, run_tags: Option<Vec<String>>) -> CommandResult<TestSummary> {
      let config = read_config(&self.app)?;
      
      // 核心过滤逻辑：
      // 如果前端传了 tags，就只保留包含这些 tag 的测试项；否则执行全部。
      let tests_to_run: Vec<TestGroup> = match run_tags {
          Some(tags) if !tags.is_empty() => {
              config.tests.into_iter().filter(|group| {
                  // 只要 group.tags 中包含 run_tags 中的任意一个，就保留
                  group.tags.iter().any(|t| tags.contains(t))
              }).collect()
          },
          _ => config.tests, // 没传 tag，跑全部
      };
  ...
  ```
* **前端 UI 适配 (`App.svelte` & `MainView.svelte`)**：前端可以增加一个下拉框或按钮组，让用户选择测试场景。
  ```svelte
  <!-- 伪代码 -->
  <select bind:value={selectedTag}>
    <option value="">全量出厂测试 (Production)</option>
    <option value="quick_check">快速健康检查 (Quick Check)</option>
    <option value="battery">仅测试电池模块 (Battery)</option>
    <option value="motor">仅测试电机模块 (Motor)</option>
  </select>

  <button on:click={() => startTest(selectedTag ? [selectedTag] : [])}>
    开始测试
  </button>
  ```
